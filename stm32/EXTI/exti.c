...
void EXTI_Init (void)
{
	RCC->APB2ENR |= RCC_APB2ENR_IOPAEN; //Тактирование GPIOB
	RCC->APB2ENR |= RCC_APB2ENR_AFIOEN; //Тактирование AFIO

	uint8_t pin_A8 = 8;
	uint8_t offset = pin_A8 * 4;						//	1 * 4 = 4 (смещение до битов отвечающий за настройку пина)
...
//	 GPIOB->CRL |= ( INPUT_FLOATING << offset );		//	записать 4 бита
	...
	GPIOB->BSRR = ( 1 << pin_A8 );						//	установка линии в 1 (диод не светится)
//	GPIOB->BRR = ( 1 << pin_A8 );						//	установка линии TX2 в 0 (диод светится)
