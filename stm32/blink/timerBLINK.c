...
// если мы считаем микросекунды в прерывании, то точность падает.
	// прерывание 1 раз в милисекунду
...
{
	static uint16_t us = 0;			//	микросекунды	(слишком большое отклонение. лучше не использовать)
	static uint16_t ms = 0;			//	милисекунды
	static uint16_t s  = 0;			//	секунды

//	us++;
	ms++;
//	if(us == 1000)	{us = 0; ms++;}	//	если набежала милисекунда	//	сбрасываем микросекунды	//	считаем милисекунды
	if(ms == 1000)	{ms = 0; s++;}	//	если набежала секунда		//	сбрасываем милисекунды	//	считаем секунды
